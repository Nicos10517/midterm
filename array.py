...
class Array:
    ...
    def __init__(self, size: int = 0, default_item_value: Any = None) -> None:
        ...
        self.array = np.full(size, default_item_value, dtype= object)
    ...
    def from_list(list_items: list) -> 'Array':
        ...
        if not isinstance(list_items, list):
            raise TypeError("list_items must be of type list")
        array_instance = Array(size = len(list_items))
        array_instance.array = np.array(list_items, dtype = object)
